...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		switch (curStage)
		{
			...
			case 'mickeybg': // photonegative
				var bg:BGSprite = new BGSprite('bgs/photonegative/office', 600, 700);
	...
	public function updateScore(miss:Bool = false)
	{
		if (ratingName == '?')
		{
			scoreTxt.text = 'Score: ' + songScore + ' | Combo Breaks: ' + songMisses + ' | Average: ?' + ' | Accuracy: ' + ratingName;
		}
		else
		{
			scoreTxt.text = 'Score: ' + songScore + ' | Combo Breaks: ' + songMisses + ' | Average: ' + Math.round(averageMs) + 'ms' + ' | Accuracy: '
				...
				if (SONG.notes[Math.floor(curStep / 16)].mustHitSection == false && !note.isSustainNote)
				{
					if (!dad.stunned)
					{
						switch (Std.int(Math.abs(note.noteData)))
						{
							case 0:
								camFollow.set(dad.getMidpoint().x + 150, dad.getMidpoint().y - 100);
								camFollow.x += dad.cameraPosition[0] - cameramovingoffset;
								camFollow.y += dad.cameraPosition[1];
							case 1:
								camFollow.set(dad.getMidpoint().x + 150, dad.getMidpoint().y - 100);
								camFollow.x += dad.cameraPosition[0];
								camFollow.y += dad.cameraPosition[1] + cameramovingoffset;
							case 2:
								camFollow.set(dad.getMidpoint().x + 150, dad.getMidpoint().y - 100);
								camFollow.x += dad.cameraPosition[0];
								camFollow.y += dad.cameraPosition[1] - cameramovingoffset;
							...
							if (!boyfriend.stunned)
							{
								switch (Std.int(Math.abs(note.noteData)))
								{
									case 0:
										...
										camFollow.x += boyfriend.cameraPosition[0] - cameramovingoffsetbf;
										camFollow.y += boyfriend.cameraPosition[1];
									case 1:
										camFollow.set(boyfriend.getMidpoint().x - 150, boyfriend.getMidpoint().y - 100);
										camFollow.x += boyfriend.cameraPosition[0];
										camFollow.y += boyfriend.cameraPosition[1] + cameramovingoffsetbf;
									case 2:
										camFollow.set(boyfriend.getMidpoint().x - 150, boyfriend.getMidpoint().y - 100);
										camFollow.x += boyfriend.cameraPosition[0];
										camFollow.y += boyfriend.cameraPosition[1] - cameramovingoffsetbf;
	...
	public function RecalculateRating(badHit:Bool = false)
	{
		...
		if (ret != FunkinLua.Function_Stop)
		{
			...
			// Rating FC
			ratingFC = "";
			if (perfects > 0 && !ClientPrefs.removePerfects)
				ratingFC = "PFC";
			if (sicks > 0)
				ratingFC = "SFC";
			if (goods > 0)
